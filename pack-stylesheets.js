...
function writeTemplates() {
  const files = findFilesInDir(path.normalize(__dirname+'/src/app/'), '.css') || [];
  let map = {};
  ...
  fs.writeFileSync(path.normalize(__dirname+'/src/app/stylesheets.ts'), dataout , {encoding: 'utf-8'});
}
  ...
  for(const file of files){
      ...
      if (stats.isDirectory()){
        ...
        if(ext.match(filter) && 
          ...
          fs.existsSync(path.normalize(path.join(startPath,basefile)+'.html'))) {
